...
/**
 * @brief This class is the code that is run on the Arduino. This file sets up the lcd screen
 * that is used to setup the box when it is deployed, and has the implementation for setting
 * which lot is being recorded at deployment. But most importantly, it is the code that
 * actually uses the sensor to record whether or not a car has passed the sensor. It sends
 * this information to the Raspberry Pi which then sends the information to the database.
 */
...
/**
 * @brief Setup function which initializes the LCD and sets the modes for all necessary pins.
 * @return void
 */
void setup()
{
...
/**
 * @brief Main loop function which either sets the lot number or reads from the sensors
...
/**
 * @brief This function reads from the sensors and sends a signal to the serial port
...
/**
 * @brief This function sets the parking lot number to be used in the database.
 * It uses 2 buttons to set the lot number and a switch to set or "unset" the current lot.
 * @return void
 */
void setLotNum() {
  if (prevSwState == HIGH) {
